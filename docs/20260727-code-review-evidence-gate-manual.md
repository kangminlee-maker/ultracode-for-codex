...
> **상태 (2026-07-27 갱신):** 이 문서는 **0.6.1 시점의 동작**을 기록합니다. 여기서 진단한 문제들은
> `docs/20260727-request-contract-and-failure-mode-design.md`의 설계로 `main` 작업 브랜치에
> 구현되었습니다. 달라진 점: ① 요청 인자 오타·잘못된 값·해석 불가 `diffBaseRef`는 지출 전에
> 거부됩니다(4.1의 "조용히 무시" 항목은 더 이상 사실이 아님) ② 게이트 실패 메시지가 어떤 규칙이
> 어떤 경로를 드롭했는지 말합니다(4.5) ③ `run --validate --name code-review`가 토큰 0으로 사전
> 확인을 제공합니다 ④ `--evidence-scope all`로 확장자 허용목록만 완화해 `.java` 등을 리뷰할 수
> 있습니다(4.6-(1)의 우회 필요성 해소) ⑤ 유효한 `diffBaseRef`만으로 커밋 구간 리뷰가
> 가능합니다(4.6-(4) 해소) ⑥ ref 문법 실수는 정규화로 살아나고, 경로가 증거에 없을 때만
> 거부됩니다. 0.6.1 설치본을 쓰는 동안에는 아래 본문이 유효합니다.
